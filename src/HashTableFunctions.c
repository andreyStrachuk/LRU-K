...
int HashTableInit (struct HashTable *table, const int size) {
    ...
    table->hash_table = (struct hash_elem *)calloc (size, sizeof (struct hash_elem));
    if (table->hash_table == NULL) return ALLOC_FAILED;

    for (int i = 0; i < size; i++) {
        create_list (table->hash_table[i].history);
    }
...
int HashTableInsert (struct HashTable *table, int value) {
    ...
    unsigned int hash = HashCRC32(table, &value);

    int res = list_search (table->hash_table[hash % table->size].history, value);

    if (res == -1)
        push_tail (table->hash_table[hash % table->size].history, value);
...
int HashTableSearch (struct HashTable *table, int value) {
    ...
    int hash = HashCRC32 (table, &value);
...
int HashCRC32 (struct HashTable *table, void *val) {
    ...
    for (int symb = 0; symb < 4; symb++)
        crc = CRCTable [(crc ^ *(value + symb)) & 0xFF] ^ (crc >> 8);
